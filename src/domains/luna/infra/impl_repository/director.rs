use crate::domains::luna::{
    domain::{Director, DirectorAffinityRepository, DirectorRepository},
    dto::{
...
// (raise it to push single-work directors further down); `AFFINITY_GAMMA`
// controls how much the absolute-volume factor lifts directors with many works
...
/// Raw row for the affinity-ordered director query. Mirrors the `director`
/// columns projected by the SELECT; the computed `score` column is used only
/// for `ORDER BY` and is intentionally not mapped here.
#[derive(Debug, FromQueryResult)]
struct AffinityDirectorRow {
    id: i64,
    name: String,
    link: String,
    manual: bool,
...
impl From<AffinityDirectorRow> for Director {
    fn from(row: AffinityDirectorRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            link: row.link,
...
#[async_trait]
impl DirectorAffinityRepository for DirectorRepo {
    async fn find_list_paginated_by_affinity(
        ...
        let count_sql = format!("SELECT COUNT(*) AS cnt FROM director d{count_where}");
        ...
        // (with diminishing returns) lifts directors with many viewed/liked
        // works. The score is a relative ordering key and is NOT bounded to
        // [0, 1]. The outer CASE guards total=0; both rate denominators are
        // always positive (M_V, M_L > 0) so there is no division by zero.
        // Directors relate to records via record.director_id (foreign key), so
        // the aggregate groups records directly (no junction table).
        let select_sql = format!(
            "SELECT d.id, d.name, d.link, d.manual, \
             COALESCE( \
             ...
             FROM director d \
             LEFT JOIN ( \
               SELECT r.director_id AS entity_id, \
               ...
               GROUP BY r.director_id \
             ) agg ON agg.entity_id = d.id\
             {where_clause} \
        ...
        let rows = AffinityDirectorRow::find_by_statement(Statement::from_sql_and_values(
        ...
        .all(db)
        .await?;
        let results: Vec<Director> = rows.into_iter().map(Director::from).collect();
