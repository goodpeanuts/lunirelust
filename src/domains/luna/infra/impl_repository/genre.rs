use crate::domains::luna::{
    domain::{Genre, GenreAffinityRepository, GenreRepository},
    dto::{
...
// (raise it to push single-work genres further down); `AFFINITY_GAMMA`
// controls how much the absolute-volume factor lifts genres with many works
...
impl From<AffinityGenreRow> for Genre {
    fn from(row: AffinityGenreRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            link: row.link,
...
#[async_trait]
impl GenreAffinityRepository for GenreRepo {
    async fn find_list_paginated_by_affinity(
        ...
        let count_sql = format!("SELECT COUNT(*) AS cnt FROM genre g{count_where}");
        ...
        // (with diminishing returns) lifts genres with many viewed/liked works.
        ...
        // Genres relate to records many-to-many via record_genre, so the
        // aggregate groups the junction rows (like idol_participation).
        let select_sql = format!(
            "SELECT g.id, g.name, g.link, g.manual, \
             COALESCE( \
             ...
             LEFT JOIN ( \
               SELECT rg.genre_id, \
                      COUNT(DISTINCT rg.record_id) AS total, \
                      COUNT(DISTINCT rg.record_id) FILTER (WHERE uri.viewed) AS viewed, \
                      COUNT(DISTINCT rg.record_id) FILTER (WHERE uri.liked AND uri.viewed) AS liked \
               FROM record_genre rg \
               LEFT JOIN user_record_interaction uri \
                      ON uri.record_id = rg.record_id AND uri.user_id = $1 \
               GROUP BY rg.genre_id \
             ) agg ON agg.genre_id = g.id\
             {where_clause} \
        ...
        let rows = AffinityGenreRow::find_by_statement(Statement::from_sql_and_values(
        ...
        .all(db)
        .await?;
        let results: Vec<Genre> = rows.into_iter().map(Genre::from).collect();
