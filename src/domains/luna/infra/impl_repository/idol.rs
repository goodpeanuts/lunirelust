use crate::domains::luna::{
    domain::{Idol, IdolAffinityRepository, IdolRepository},
    dto::{
...
// (raise it to push single-work idols further down); `AFFINITY_GAMMA` controls
// how much the absolute-volume factor lifts idols with many works (raise it to
// reward large catalogs more). See the score expression below for how they
// combine.
...
impl From<AffinityIdolRow> for Idol {
    fn from(row: AffinityIdolRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            link: row.link,
...
#[async_trait]
impl IdolAffinityRepository for IdolRepo {
    async fn find_list_paginated_by_affinity(
        ...
        let count_sql = format!("SELECT COUNT(*) AS cnt FROM idol i{count_where}");
        ...
        // (with diminishing returns) lifts idols with many viewed/liked works.
        ...
        let select_sql = format!(
            "SELECT i.id, i.name, i.link, i.manual, \
             COALESCE( \
             ...
             LEFT JOIN ( \
               SELECT ip.idol_id, \
                      COUNT(DISTINCT ip.record_id) AS total, \
                      COUNT(DISTINCT ip.record_id) FILTER (WHERE uri.viewed) AS viewed, \
                      COUNT(DISTINCT ip.record_id) FILTER (WHERE uri.liked AND uri.viewed) AS liked \
               FROM idol_participation ip \
               LEFT JOIN user_record_interaction uri \
                      ON uri.record_id = ip.record_id AND uri.user_id = $1 \
               GROUP BY ip.idol_id \
             ) agg ON agg.idol_id = i.id\
             {where_clause} \
        ...
        let rows = AffinityIdolRow::find_by_statement(Statement::from_sql_and_values(
        ...
        .all(db)
        .await?;
        let results: Vec<Idol> = rows.into_iter().map(Idol::from).collect();
