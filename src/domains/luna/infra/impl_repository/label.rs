use crate::domains::luna::{
    domain::{Label, LabelAffinityRepository, LabelRepository},
    dto::{
...
// (raise it to push single-work labels further down); `AFFINITY_GAMMA` controls
// how much the absolute-volume factor lifts labels with many works (raise it to
...
impl From<AffinityLabelRow> for Label {
    fn from(row: AffinityLabelRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            link: row.link,
...
#[async_trait]
impl LabelAffinityRepository for LabelRepo {
    async fn find_list_paginated_by_affinity(
        ...
        let count_sql = format!("SELECT COUNT(*) AS cnt FROM label l{count_where}");
        ...
        // (with diminishing returns) lifts labels with many viewed/liked works.
        ...
        // Labels relate to records via record.label_id (foreign key), so the
        // aggregate groups records directly (no junction table).
        let select_sql = format!(
            "SELECT l.id, l.name, l.link, l.manual, \
             COALESCE( \
             ...
             FROM label l \
             LEFT JOIN ( \
               SELECT r.label_id AS entity_id, \
        ...
        let rows = AffinityLabelRow::find_by_statement(Statement::from_sql_and_values(
        ...
        .all(db)
        .await?;
        let results: Vec<Label> = rows.into_iter().map(Label::from).collect();
