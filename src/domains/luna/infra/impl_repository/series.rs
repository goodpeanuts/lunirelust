use crate::domains::luna::{
    domain::{Series, SeriesAffinityRepository, SeriesRepository},
    dto::{
...
// (raise it to push single-work series further down); `AFFINITY_GAMMA` controls
// how much the absolute-volume factor lifts series with many works (raise it to
...
impl From<AffinitySeriesRow> for Series {
    fn from(row: AffinitySeriesRow) -> Self {
        Self {
            id: row.id,
            name: row.name,
            link: row.link,
...
#[async_trait]
impl SeriesAffinityRepository for SeriesRepo {
    async fn find_list_paginated_by_affinity(
        ...
        let count_sql = format!("SELECT COUNT(*) AS cnt FROM series s{count_where}");
        ...
        // (with diminishing returns) lifts series with many viewed/liked works.
        ...
        // Series relate to records via record.series_id (foreign key), so the
        // aggregate groups records directly (no junction table).
        let select_sql = format!(
            "SELECT s.id, s.name, s.link, s.manual, \
             COALESCE( \
             ...
             FROM series s \
             LEFT JOIN ( \
               SELECT r.series_id AS entity_id, \
        ...
        let rows = AffinitySeriesRow::find_by_statement(Statement::from_sql_and_values(
        ...
        .all(db)
        .await?;
        let results: Vec<Series> = rows.into_iter().map(Series::from).collect();
